...
export default function App() {
  ...
  const createLog = async (action) => {
    ...
    try {
      await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'logs'), {
        action, user: user?.name || 'Sistema', timestamp: new Date().toISOString()
      });
  ...
  // ✅ NOVO: Exclusão de aluno direto pelo card
  ...
  const handleStudentDesmarcar = async (scheduleId, hour) => {
    ...
    let creditBonus = 0;
    if (diffHours >= 4) { newStatus = 'desmarcado'; creditBonus = 1; }
  ...
  const handleReposicao = async (hour) => {
    ...
    await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'schedules'), {
      name: user.name, studentId: user.id, day: selectedDay, hour: hour,
      status: 'reposicao', createdBy: user.name, createdAt: new Date().toISOString()
    });
          ...
          <div>
            <div className="flex flex-col md:flex-row justify-between gap-6 mb-12">
              ...
              <div className="flex gap-4">
                <input type="text" placeholder="Buscar aluno..." className="bg-[#11141a] border border-white/5 rounded-2xl px-6 py-4 text-xs w-64 outline-none text-white" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
              ...
              {students.filter(s => s.name.toLowerCase().includes(searchTerm.toLowerCase())).map(s => (
                <div key={s.id} className="bg-[#11141a] p-6 rounded-[2.5rem] border border-white/5 hover:border-emerald-500/30 cursor-pointer transition-all group relative">
                  ...
                  <div onClick={() => setShowStudentDetailsId(s.id)}>
                    ...
                    <h3 className="font-black uppercase text-sm mb-1 truncate pr-16">{s.name}</h3>
                  ...
                  {/* ✅ Botões Editar + Excluir — aparecem no hover no canto superior direito */}
                  <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-all">
                    <button 
                      onClick={(e) => { e.stopPropagation(); setEditStudentData(JSON.parse(JSON.stringify(s))); }}
                      className="p-2 bg-white/5 text-gray-500 rounded-xl hover:bg-emerald-500 hover:text-black transition-all"
                      title="Editar aluno"
                    >
                      <Edit3 size={15}/>
                    </button>
                    <button 
                      ...
                      className="p-2 bg-white/5 text-gray-500 rounded-xl hover:bg-rose-500 hover:text-white transition-all"
                      title="Excluir aluno"
                    >
                      <Trash2 size={15}/>
                    </button>
            ...
            <form onSubmit={handleAddStudent} className="space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-1">
                  ...
                  <select className="w-full bg-[#1a1f26] border border-white/5 rounded-2xl p-5 text-sm text-white" value={newStudent.plan} onChange={e => setNewStudent({...newStudent, plan: e.target.value})}>
                    {PLANOS.map(p => <option key={p} value={p}>{p}</option>)}
                  </select>
                </div>
                <div className="space-y-1">
                  <label className="text-[9px] font-black uppercase text-gray-600 ml-2">Frequência</label>
                  <select className="w-full bg-[#1a1f26] border border-white/5 rounded-2xl p-5 text-sm text-white" value={newStudent.frequency} onChange={e => {
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  {newStudent.fixedSchedules.map((sched, idx) => (
                    <div key={idx} className="bg-[#1a1f26] p-5 rounded-2xl border border-white/5 space-y-3">
                      <select className="w-full bg-black/40 border border-white/5 rounded-xl p-3 text-xs text-white" value={sched.day} onChange={e => {
                      ...
                      <select className="w-full bg-black/40 border border-white/5 rounded-xl p-3 text-xs text-white" value={sched.hour} onChange={e => {
                        const copy = [...newStudent.fixedSchedules];
        ...
        <div className="fixed inset-0 bg-black/95 z-[250] flex items-center justify-center p-4 backdrop-blur-xl">
          <div className="bg-[#11141a] w-full max-w-4xl rounded-[3.5rem] p-10 border border-white/10 overflow-y-auto max-h-[90vh] no-scrollbar shadow-2xl">
            ...
            <form onSubmit={handleEditStudent} className="space-y-8">
              ...
              <button type="submit" className="w-full bg-emerald-500 text-black font-black py-5 rounded-2xl uppercase text-xs flex items-center justify-center gap-2">
...
function InputGroup({ label, type = 'text', value, onChange, required = false }) {
  return (
    <div className="space-y-1">
      ...
      <input type={type} className="w-full bg-[#1a1f26] border border-white/5 rounded-2xl p-5 text-sm text-white outline-none focus:border-emerald-500/30 transition-all" value={value || ''} onChange={e => onChange(e.target.value)} required={required} />
    </div>
  );
}
